...
const singles = [
  ...
  [{
    name: "Eat at Moe's"
  }, "name = 'Eat at Moe\\\'s'"], // eslint-disable-line no-useless-escape
  [{
    names: "Foo.bar"
  }, "name = 'Foo.bar'"],
  [{
    fullText: "complete"
  }, "fullText contains 'complete'"],
  [{
    mimeType: "text/csv"
  }, "mimeType = 'text/csv'"],
  [{
  ...
  [{
    writer: 'xyz@gmail'
  }, "'xyz@gmail' in writers"],
  [{
    writers: 'xyz@gmail'
  }, "'xyz@gmail' in writers"],
  [{
    reader: 'qwerty@gmail'
  }, "'qwerty@gmail' in readers"],
  [{
    readers: 'qwerty@gmail'
  }, "'qwerty@gmail' in readers"],
  [{
    sharedWithMe: true
  }, "sharedWithMe = true"],
  ...
  }, "properties has { key='role' and value='tick data' }"],
  [{
    properties: {
      x: 35
    }
  }, "properties has { key='x' and value='35' }"],
  [{
    appProperties: {
      paid: true
    }
  }, "appProperties has { key='paid' and value='true' }"],
  [{
    visibility: "limited"
  }, "visibility = 'limited'"],
  [{
...
describe('search-string-for-google-drive: ', function() {
  ...
  describe('ssgd.extract(obj) will create an object extracting supported terms only from obj ', function() {
    ...
    it('{junk:1, trashed: 1} --> (ssgd.extract) --> { trashed: 1 } ', function() {
      ssgd.extract(mix).should.deepEqual({
        trashed: 1
      });
    });
  });
